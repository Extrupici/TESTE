...
const start = (dxxoo = new Client()) => {
	...
	await dxxoo.sendText(chat.id, `Desculpe, o grupo neste Bot está cheio\nMax Group is: ${groupLimit}`).then(() => {
	...
	// kondisi ketika batas member group belum tercapai, ubah di file settings/setting.json
	    ...
	    await dxxoo.sendText(chat.id, `Desculpa, O bot sai se os membros do grupo não excederem ${memberLimit} people`).then(() => {
	      dxxoo.leaveGroup(chat.id)
	      dxxoo.deleteChat(chat.id)
	    })
	    } else {
    ...
    dxxoo.onGlobalParicipantsChanged(async (event) => {
        ...
        if (event.action === 'add' && event.who !== host && isWelcome) {
            ...
            await dxxoo.sendTextWithMentions(event.chat, `Ola, Bem vindo ao grupo, agora mama o bonde @${event.who.replace('@c.us', '')} \n\nHave fun with us✨`)
        }
        // kondisi ketika seseorang dikick/keluar dari group
        if (event.action === 'remove' && event.who !== host) {
			await dxxoo.sendFileFromUrl(event.chat, profile, 'profile.jpg', '')
            await dxxoo.sendTextWithMentions(event.chat, `Adeus ja vai tarde em mano @${event.who.replace('@c.us', '')}, We'll miss you✨`)
        }
    })

    dxxoo.onIncomingCall(async (callData) => {
        // ketika seseorang menelpon nomor bot akan mengirim pesan
        await dxxoo.sendText(callData.peerJid, 'Não foi possível receber chamadas. \ N \ n-bot)
        .then(async () => {
